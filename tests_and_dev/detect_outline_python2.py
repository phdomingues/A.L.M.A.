...
while True:
    ...
    if patient_face:
        ...
        if mouth_pos:
            ...
            general = np.concatenate((histR,histG,histB,histGray), axis = None)
            ...
            # plot mean line
            plt.plot([np.mean(general)]*256, color = 'black', linewidth=3.0)
            ...
            print("All   : max: {:d} \t second max: {:d} \t average: {:.2f}  \t Standard Deviation: {:.2f} \t Variance: {:.2f}".format(np.max(general), np.sort(general)[-2], np.mean(general), np.std(general), np.var(general)))
            #######################
