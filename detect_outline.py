...
while True:
    ...
    if patient_face:
        ...
        if mouth_pos:
            ...
            print("-"*50)
            print("Blue  : max: {:d} \t second max: {:d} \t average: {:.2f}  \t Standard Deviation: {:.2f} \t Variance: {:.2f}".format(np.max(histB), np.sort(histB)[-2], np.average(histB,weights = range(256)), np.std(histB), np.var(histB)))
            print("Green : max: {:d} \t second max: {:d} \t average: {:.2f}  \t Standard Deviation: {:.2f} \t Variance: {:.2f}".format(np.max(histG), np.sort(histG)[-2], np.average(histG,weights = range(256)), np.std(histG), np.var(histG)))
            print("Red   : max: {:d} \t second max: {:d} \t average: {:.2f}  \t Standard Deviation: {:.2f} \t Variance: {:.2f}".format(np.max(histR), np.sort(histR)[-2], np.average(histR,weights = range(256)), np.std(histR), np.var(histR)))
            print("Gray  : max: {:d} \t second max: {:d} \t average: {:.2f}  \t Standard Deviation: {:.2f} \t Variance: {:.2f}".format(np.max(histGray), np.sort(histGray)[-2], np.average(histGray,weights = range(256)), np.std(histGray), np.var(histGray)))
